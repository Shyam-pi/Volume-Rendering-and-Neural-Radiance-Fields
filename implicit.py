...
class NeuralRadianceField(torch.nn.Module):
    def __init__(
        ...
        self.input_ch = embedding_dim_xyz

        self.inter_mlp = torch.nn.ModuleList(
            [torch.nn.Linear(self.input_ch, self.num_c)] + [torch.nn.Linear(self.num_c, self.num_c) 
                                                            if i not in self.skips else torch.nn.Linear(self.num_c + self.input_ch, self.num_c) 
                                                            for i in range(self.num_layers-1)])

        self.color_linear = torch.nn.Linear(self.num_c, 3)
        self.alpha_linear = torch.nn.Linear(self.num_c, 1)

    def forward(self, raybundle:RayBundle):
        sample_pts = raybundle.sample_points
        input_pts = self.harmonic_embedding_xyz(sample_pts)

        # print(f"MLP input shape = {input_pts.shape}")

        h = input_pts
        for i, l in enumerate(self.inter_mlp):
            h = self.inter_mlp[i](h)
            h = F.relu(h)
            if i in self.skips:
                h = torch.cat([input_pts, h], -1)

        rgb = F.sigmoid(self.color_linear(h))
