...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            # print(grid_vis.min())
            # print(grid_vis.max())
            grid_vis = normalize(grid_vis)
            plt.imsave(
                f'images/grid_vis.png',
                grid_vis
            )
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            # print(ray_vis.min())
            # print(ray_vis.max())
            ray_vis = normalize(ray_vis)
            plt.imsave(
                f'images/ray_vis.png',
                ray_vis
            )
        ...
        if cam_idx == 0 and file_prefix == '':
            pcl = ray_bundle.sample_points.view(1,-1,3)
            # print(pcl.shape)
            rend_img = render_points("images/pcl_vis.png", pcl)
        ...
        if cam_idx == 2 and file_prefix == '':
            ...
            # print(f"depth map max = {depth_image.max()}, depth map min = {depth_image.min()}")

            plt.imsave(
                f'images/depth_map.png',
                depth_image
            )
...
def render(
    ...
    all_images = render_images(
        model, cameras, cfg.data.image_size, cfg=cfg
    )
...
def train(
    ...
    # Print center and side lengthscriterion = torch.nn.MSELoss()
    ...
    all_images = render_images(
        model, create_surround_cameras(3.0, n_poses=20), image_size, cfg=cfg, file_prefix='part_2'
    )
        ...
        if (
            ...
            with torch.no_grad():
                test_images = render_images(
                    ...
                    cfg.data.image_size, file_prefix='nerf',cfg=cfg
                )
                imageio.mimsave(f'images/part_3_epochs_{epoch}.gif', [np.uint8(im * 255) for im in test_images], loop = 0)
